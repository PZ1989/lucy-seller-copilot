...
export function DraftWorkflow({ mode, title, t, language = "en", onBack, listingId, initialListing, onChangeListing, changeListingLabel }: Props) {
  ...
  const [provider, setProvider] = useState<"auto" | "openai" | "claude">("auto");
  ...
  async function generate() {
    ...
    try {
      const result = await generateLucyListing(prompt, language, form.type === "download" ? "download" : "physical", provider === "auto" ? undefined : provider);
  ...
  async function optimize() {
    ...
    try { setOptimization(await optimizeLucyListing({ title: form.title, description: form.description, tags: form.tags ?? [], materials: form.materials ?? [], productType: form.type === "download" ? "digital" : "physical", language, provider: provider === "auto" ? undefined : provider })); } catch { setError(t("listings.generation.error")); } finally { setOptimizing(false); }
  }
  ...
  if (mode === "lucy" && !generated) return <LucyStart t={t} prompt={prompt} setPrompt={setPrompt} provider={provider} setProvider={setProvider} generating={generating} error={error} onBack={onBack} onGenerate={() => void generate()} />;
