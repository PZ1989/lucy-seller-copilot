...
export default function BillingPage() {
  ...
  const monthlyPercent = credits?.monthlyAllowance ? credits.monthlyRemaining / credits.monthlyAllowance : 0;
  const creditMessage = credits?.totalAvailable === 0 ? t("billing.credits.exhausted") : monthlyPercent <= 0.1 ? t("billing.credits.veryLow") : monthlyPercent <= 0.2 ? t("billing.credits.low") : "";
  ...
  return <main className="min-h-screen w-full px-4 pb-12 pt-6 sm:px-6 lg:px-8 lg:pt-8"><div className="mx-auto w-full max-w-[1120px]">
    ...
    {data && <>
      ...
      {credits && <section className="mt-6 rounded-[16px] border border-charcoal/[0.07] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-terracotta">AI Credits</p><p className="mt-2 text-2xl font-semibold text-charcoal">{credits.totalAvailable.toLocaleString()} / {credits.monthlyAllowance.toLocaleString()} <span className="text-[12px] font-normal text-charcoal/45">{t("billing.credits.available")}</span></p></div><div className="text-right text-[11px] text-charcoal/50"><p>{t("billing.credits.purchased")}: {credits.purchasedRemaining.toLocaleString()}</p><p>{credits.billingCycleEndsAt ? `${t("billing.credits.renews")}: ${new Intl.DateTimeFormat().format(new Date(credits.billingCycleEndsAt))}` : ""}</p></div></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#F4F2F0]"><div className="h-full rounded-full bg-terracotta" style={{ width: `${Math.max(0, Math.min(100, 100 - credits.usagePercent))}%` }}/></div>{creditMessage && <p className="mt-3 text-[11px] font-semibold text-[#B34A35]">{creditMessage}</p>}<button type="button" onClick={() => setTopupOpen(true)} className="mt-4 inline-flex h-8 items-center rounded-[9px] bg-terracotta px-3 text-[10px] font-semibold text-white">{t("billing.credits.buy")}</button></section>}
      <section className="mt-6 rounded-[14px] border border-charcoal/[0.07] bg-[#FCFAF8] p-4 text-[11px] leading-5 text-charcoal/60"><p className="font-semibold text-charcoal">{t("billing.credits.explanationTitle")}</p><p className="mt-1">{t("billing.credits.explanation")}</p></section>
      ...
      {topupOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-charcoal/30 p-4"><div role="dialog" aria-modal="true" className="w-full max-w-md rounded-[18px] bg-white p-5"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-charcoal">{t("billing.credits.buy")}</h2><button type="button" onClick={() => setTopupOpen(false)} className="text-[11px] text-charcoal/50">{t("billing.credits.close")}</button></div><div className="mt-4 grid gap-3">{[["500", "$9", false], ["1500", "$19", true], ["5000", "$49", false]].map(([amount, price, recommended]) => <div key={String(amount)} className="flex items-center justify-between rounded-[12px] border border-charcoal/[0.08] p-3"><div><p className="text-[13px] font-semibold text-charcoal">{amount} AI credits</p>{recommended && <p className="text-[10px] text-terracotta">{t("billing.credits.recommended")}</p>}</div><button type="button" disabled className="rounded-[9px] bg-charcoal/10 px-3 py-2 text-[10px] text-charcoal/45">{price}</button></div>)}</div><p className="mt-4 text-[11px] leading-5 text-charcoal/55">{t("billing.credits.finalizing")}</p></div></div>}
    </>}
  </div></main>;
}
