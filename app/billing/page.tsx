...
const planDetails: Record<PlanId, { name: string; price: string; limit: string }> = {
  pro: { name: "PRO", price: "$59", limit: "1" },
  multi: { name: "MULTI STORE", price: "$99", limit: "5" },
};
...
export default function BillingPage() {
  ...
  function load() {
    ...
    void getBillingStatus().then(setData).catch((requestError) => setError(requestError instanceof ApiError ? requestError : new ApiError("Could not load billing status.", "unavailable"))).finally(() => setLoading(false));
  }
  useEffect(() => { load(); }, []);
