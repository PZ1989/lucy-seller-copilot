...
export default function ConnectionsPage() {
  ...
  const refresh = useCallback(async () => { try { const session = await getSession(); if (!session?.user) throw new ApiError("unauthenticated", "unauthenticated"); const [etsyResult, aiResult] = await Promise.allSettled([getEtsyConnection(), getLucyConnections()]); if (etsyResult.status === "fulfilled") { setEtsy(etsyResult.value); setError(""); } else if (!etsy || !etsyResult.reason) { setError(t("connections.error.check")); } if (aiResult.status === "fulfilled") setAi(aiResult.value); } catch { if (!etsy) setError(t("connections.error.check")); } finally { setLoading(false); } }, [etsy, t]);
